...
void CheckMemoryLeakInFunction::checkScope(const Token *startTok, const std::string &varname, unsigned int varid, bool classmember, unsigned int sz)
{
    ...
    if ((result = findleak(tok)) != nullptr) {//Key_Point
...
void CheckMemoryLeakInFunction::checkMemset()
{
	...
	for (std::size_t i = 0; i < functions; ++i) {
		...
		for (const Token *tok = scope->classStart; tok && tok != scope->classEnd; tok = tok->next()) {
			if (Token::Match(tok, "memset|memcpy|memmove ( %any%")) {
				...
				const Token* arg2 = arg1->nextArgument();
				...
				if (!arg3)
					continue;
				/*TD->*/
				const Token *strtoken1 = arg1->getValueTokenMaxStrLength();
				const Token *strtoken2 = arg2->getValueTokenMaxStrLength();
				if (Token::Match(arg3, "strlen ( %var%"))
					if (strtoken1 && strtoken2 && Token::getStrLength(strtoken1) < Token::getStrLength(strtoken2)){
					reportError(tok, Severity::error, "memoryLeak", "Unsafe memory operater" + tok->str() + "arg is out of bounds");
				}
				/*<-TD*/
