...
void CheckBufferOverrun::valueFlowCheckArrayIndex(const Token * const tok, const ArrayInfo &arrayInfo)
{
    ...
    */
    ...
    for (int warn = 0; warn == 0 || warn == 1; ++warn) {
        // Negative index..
        for (const Token *tok2 = tok; tok2 && tok2->str() == "["; tok2 = tok2->link()->next()) {//check array like "str[][]"
...
//TD
void CheckBufferOverrun::checkarrayOutOfBoundsInWhileLoop()
{
	...
	for (std::list<Scope>::const_iterator scope = symbolDatabase->scopeList.begin(); scope != symbolDatabase->scopeList.end(); ++scope) {
		...
		if (Token::Match(tok, "%var%[.%var%] <|<= %num%") && tok->tokAt(2)->str() != "null"){
			vvtok = tok;
			...
			for (const Token *vtok = bodyStart->next(); vtok != bodyStart->link(); vtok = vtok->next()){
				...
				a = Token::Match(vtok, "%var% [");
				b = Token::Match(vtok->tokAt(2), "%varid% ] =", vvtok->varId());
				const Variable *var = vtok->variable();
				if (var && var->isArray() && var->dimensions().size() == 1) {
					MathLib::bigint len = var->dimension(0);
					if (a&&b){
						if (len <= valueMax)
							bufferOverrunError(vtok, tok->str());
					}
				}
			}
		}
