...
void Variable::evaluate(const Library* lib)
{
	...
	if (_start) {
		...
		if (Token::Match(_start, "bool")){
			_dataType = IsboolType;
			setFlag(fIsIntType, true);
		}
		else if (Token::Match(_start, "char")){
			_dataType = IscharType;
			setFlag(fIsIntType, true);
		}
		else if (Token::Match(_start, "short")){
			_dataType = IsshortType;
			setFlag(fIsIntType, true);
		}
		else if (Token::Match(_start, "int")){
			_dataType = IsIntType;
			setFlag(fIsIntType, true);
		}
		else if (Token::Match(_start, "long")){
			_dataType = IsLongType;
			setFlag(fIsIntType, true);
		}
		else if (Token::Match(_start, "float")){
			_dataType = IsFloatType;
			setFlag(fIsFloatType, true);
		}
		else if (Token::Match(_start, "double")){
			_dataType = IsDoubleType;
			setFlag(fIsFloatType, true);
		}
	}
}
