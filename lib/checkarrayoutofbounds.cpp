...
#include "checkarrayoutofbounds.h"
...
namespace {
	CheckArrayOutOfBunds instance;
}

//---------------------------------------------------------------------------

static void makeArrayIndexOutOfBoundsError(std::ostream& oss, const CheckArrayOutOfBunds::ArrayInfo &arrayInfo, const std::vector<MathLib::bigint> &index)
{
...
void CheckBufferOverrun::checkGlobalAndLocalVariable()
{
	...
	for (const Token *tok = _tokenizer->tokens(); tok; tok = tok->next()) {
		if (Token::Match(tok, "%str% [ %num% ]")) {
			std::cout << "...." << tok->str() << std::endl;//added by TD
...
void CheckBufferOverrun::bufferOverrun2()
{
	...
	for (const Token *tok = _tokenizer->tokens(); tok; tok = tok->next()) {
		...
		if (index)
			std::cout << "TT:" << index->str() << std::endl;//TD
...
//---------------------------------------------------------------------------
//TD
void CheckBufferOverrun::bufferOverrun3()
{
	// singlepass checking using ast, symboldatabase and valueflow
	for (const Token *tok = _tokenizer->tokens(); tok; tok = tok->next()) {
