...
public:
    ...
    struct AllocInfo {
        AllocStatus status;
        int type;
		AllocInfo(int type_ = 0, AllocStatus status_ = NOALLOC)
			: status(status_), type(type_){}
    };
...
/*TD*/
class CPPCHECKLIB PointerSet {
public:
	enum AllocStatus { DEALLOC = -1, NOALLOC = 0, ALLOC = 1 };
		AllocStatus status;
		unsigned int stackLen;
		unsigned int id;
	std::map<const Token*,MathLib::bigint> pointer;
};

static std::list<PointerSet> pointerSetList;
...
private:
    ...
    void checkScope(const Token * const startToken, VarInfo *varInfo,
                    std::set<unsigned int> notzero);
	/*check pointer leak*/
	void checkMallocStack(const Token * const startToken);//TD
	void checkMallocStackLeak();//TD
	const Token* findTokenById(const unsigned int varid);
    /** parse function call */
	...
	//void deallocError(const Token *tok, const std::string &varname);
