...
static void valueFlowForLoop(TokenList *tokenlist, SymbolDatabase* symboldatabase, ErrorLogger *errorLogger, const Settings *settings)
{
    for (std::list<Scope>::const_iterator scope = symboldatabase->scopeList.begin(); scope != symboldatabase->scopeList.end(); ++scope) {
        ...
        if (!Token::simpleMatch(tok->next()->astOperand2(), ";") ||
            !Token::simpleMatch(tok->next()->astOperand2()->astOperand2(), ";"))//(int i=0;i<22;i++)
            continue;
...
static void valueFlowSwitchVariable(TokenList *tokenlist, SymbolDatabase* symboldatabase, ErrorLogger *errorLogger, const Settings *settings)
{
    for (std::list<Scope>::iterator scope = symboldatabase->scopeList.begin(); scope != symboldatabase->scopeList.end(); ++scope) {
		...
		Token *vtok = const_cast<Token *>(scope->classDef);
        const Variable *var = vartok->variable();
		if (!var)
			continue;
		// bailout: global non-const variables
		...
		std::list<ValueFlow::Value> values;
		for (Token *tok = vtok; tok; tok = tok->previous()){
			if (Token::Match(tok, " %varid% ", vartok->varId()) && !tok->values.empty()){
				values = tok->values;
				...
				valueFlowForward(const_cast<Token*>(scope->classDef->tokAt(2)), vartok->variable()->scope()->classEnd, vartok->variable(), vartok->varId(), values, false, tokenlist, errorLogger, settings);
				break;
			}
		}
        ...
        for (Token *tok = scope->classStart->next(); tok != scope->classEnd; tok = tok->next()) {
			...
			if (Token::Match(tok->tokAt(-2), "%var% = %var% ;")) {
				std::list<ValueFlow::Value> values;
				for (Token *stok = tok->tokAt(-2); stok; stok = stok->previous()){
					if (Token::Match(stok, " %varid% ", tok->varId())){
						if(!stok->values.empty()){
							values = stok->values;
							for (std::list<ValueFlow::Value>::const_iterator val = values.begin(); val != values.end(); ++val) {
								valueFlowReverse(tokenlist,
									const_cast<Token*>(scope->classDef),
							...
							valueFlowForward(tok, tok->variable()->scope()->classEnd, tok->variable(), tok->varId(), values, false, tokenlist, errorLogger, settings);							
						}
						else {
							if (Token::Match(stok, " %varid% = %num% ;", tok->varId())){
								values.push_back(ValueFlow::Value(MathLib::toLongNumber(stok->tokAt(2)->str())));
								for (std::list<ValueFlow::Value>::const_iterator val = values.begin(); val != values.end(); ++val) {
									valueFlowReverse(tokenlist,
										const_cast<Token*>(scope->classDef),
										vartok,
