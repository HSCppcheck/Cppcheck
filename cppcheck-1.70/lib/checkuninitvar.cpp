        ...
        if (tok->varId() == var.declarationId()) {
            ...
            if (!membervar.empty()) {
                ...
                if (!suppressErrors && isVariableUsage(tok, var.isPointer(), *alloc))
					...
					// find out if tok has possible values
					if (tok->values.empty())
					{
						uninitvarError(tok, tok->str(), *alloc);
					}
...
void CheckUninitVar::checkRhs(const Token *tok, const Variable &var, Alloc alloc, unsigned int number_of_if, const std::string &membervar)
{
    ...
    while (nullptr != (tok = tok->next())) {
        if (tok->str() == "=")
			...
			if (membervar.empty() && isVariableUsage(tok, var.isPointer(), alloc) && tok->variable() && !tok->variable()->isArgument())
                uninitvarError(tok, tok->str(), alloc);
...
{
	 reportError(tok, Severity::error, "uninitdata", "Violate rule R-1-11-1:Memory is allocated but not initialized: " + varname);
}

void CheckUninitVar::uninitvarError(const Token *tok, const std::string &varname)
{
    reportError(tok, Severity::error, "uninitvar", "Violate rule R-1-11-1:Uninitialized variable: " + varname);
}
