...
void CheckOther::checkRules()//TD
{
	enum Type{
		none = 0,
		IsboolType = (1 << 12), /** @brief Integral type */
		IscharType = (1 << 13),  /** @brief Floating point type */
		IsshortType = (1 << 14),  /** @brief Long Integral type */
		IsIntType = (1 << 15),  /** @brief Double point type */
		IsLongType = (1 << 16), /** @brief char type */
		IsFloatType = (1 << 17), /** @brief short Integral type */
		IsDoubleType = (1 << 18) /** @brief short Integral type */
	};
	...
	for (std::list<Scope>::const_iterator scope = symbolDatabase->scopeList.begin(); scope != symbolDatabase->scopeList.end(); ++scope) {
		if (scope->type != Scope::eFunction)
			continue;
		for (const Token *tok = scope->classStart->next(); tok != scope->classEnd; tok = tok->next()) {
			if (Token::Match(tok, "; %var% ;") || Token::Match(tok, "; %var% ") && Token::Match(tok->tokAt(2), " -|/|+|*|>|<|==|!=|>=|<=  %var%|%num% ;")){
				bool isForCondition = false;
				const Token *stok = tok;
				for (int i=0; i<10,stok; i++)
				{
					stok = stok->previous();
					if (Token::Match(stok, "for ( ")){
						isForCondition = true;
						break;
					}					
				}
				if (!isForCondition){
					reportError(tok->next(), Severity::error, "noneffect ", "Violate rule R-1-8-3: Noneffect statement.", 0U, false);
				}
		
				continue;
			}				
			else if (Token::Match(tok, "switch ( %var% )") && !Token::Match(tok->tokAt(5), "case ") &&!Token::Match(tok->tokAt(5), "default : ")) {				
				tok = tok->tokAt(5);
				reportError(tok, Severity::error, "unreachable", "Violate rule R-1-8-1: Unreachable code.", 0U, false);				
				continue;
			}
			else if (Token::Match(tok, "break ;") && !Token::Match(tok->tokAt(2), "case ") 
				&& !Token::Match(tok->tokAt(2), "} ") && !Token::Match(tok->tokAt(2), "default : ")) {
				tok = tok->tokAt(2);
				reportError(tok, Severity::error, "unreachable", "Violate rule R-1-8-1: Unreachable code.", 0U, false);
				continue;
			}
				...
				if (tok->next()->str() != "}" && (tok->scope()->type != Scope::eIf) &&
					(tok->scope()->type != Scope::eElse) && (tok->scope()->type != Scope::eSwitch) &&
					(tok->scope()->type != Scope::eWhile)){
					reportError(tok, Severity::error, "unreachable", "Violate rule R-1-8-1: Unreachable code.", 0U, false);
					continue;
				}
			}
	...
	//for type cast
	for (const Token *tok = _tokenizer->tokens(); tok; tok = tok->next()) {
		if (tok->tokType() == Token::eAssignmentOp && tok->astOperand1() && tok->astOperand1()->tokType() == Token::eVariable){//assignment to variable
			const Variable* var1 = tok->astOperand1()->variable();
			if (Token::Match(tok->next(), "( %type% ")){//casts
				const Token *typetok = tok->tokAt(2);
				if (Token::Match(typetok, "bool"))
					type = IsboolType;
				else if (Token::Match(typetok, "char"))
					type = IscharType;
				else if (Token::Match(typetok, "short"))
					type = IsshortType;
				else if (Token::Match(typetok, "int"))
					type = IsIntType;
				else if (Token::Match(typetok, "long"))
					type = IsLongType;
				else if (Token::Match(typetok, "float"))
					type = IsFloatType;
				else if (Token::Match(typetok, "double"))
					type = IsDoubleType;
				if (var1->isPointer() && Token::Match(tok->tokAt(3), "*")){
					if (var1->_dataType != type){
						reportError(tok, Severity::error, "type assign", "Violate rule R-1-10-4: Pointer assignment must keep type consistent.", 0U, false);
					}
				}	
				else if (!var1->isPointer()){
					if (var1->_dataType <= IsLongType && type >= IsFloatType)
						reportError(tok, Severity::error, "type assign", "Violate rule R-1-10-1: Use type cast when float var assigned to int.", 0U, false);
					else if (var1->_dataType == IsFloatType && type == IsDoubleType)
						reportError(tok, Severity::error, "type assign", "Violate rule R-1-10-3: Use type cast when double var assigned to float.", 0U, false);
					else if (var1->_dataType < type && IsboolType <= type && type == IsLongType)
						reportError(tok, Severity::error, "type assign", "Violate rule R-1-10-2: Use type cast when long var assigned to int.", 0U, false);
				}
			}					
			else{
				if (tok->astOperand2() && tok->astOperand2()->tokType() == Token::eVariable){//no cast 
					const Variable* var2 = tok->astOperand2()->variable();
					if (tok->astOperand2()->values.empty() && !var2->isArgument())//&& !tok->scope()->isExecutable()
						reportError(tok->astOperand2(), Severity::error, "uninitvar", "Violate rule R-1-11-1: Uninitialized variable:"+ var2->name(), 0U, false);
					if (!var1->isPointer() && !var2->isPointer()){ 
						if (var1->_dataType <= IsLongType && var2->_dataType >= IsFloatType)
							reportError(tok, Severity::error, "type assign", "Violate rule R-1-10-1: Use type cast when float var assigned to int.", 0U, false);
						else if (var1->isFloatingType() && var2->_dataType == IsDoubleType)
							reportError(tok, Severity::error, "type assign", "Violate rule R-1-10-3: Use type cast when double var assigned to float.", 0U, false);
						else if (var1->_dataType < var2->_dataType && IsboolType <= var2->_dataType && var2->_dataType <= IsLongType
							|| var1->_dataType == var2->_dataType &&(var1->typeEndToken()->isSigned() && var2->typeEndToken()->isUnsigned()
							|| var2->typeEndToken()->isSigned() && var1->typeEndToken()->isUnsigned()))
							reportError(tok, Severity::error, "type assign", "Violate rule R-1-10-2: Use type cast when long var assigned to int.", 0U, false);
					}					
					else if (var1->isPointer() && !var2->isArrayOrPointer())
						reportError(tok, Severity::error, "type assign", "Violate rule R-1-10-5: Forbid variable assigned to Pointer.", 0U, false);

				}
				else if (!var1->isPointer() && Token::Match(tok->astOperand2(), "& %var%"))
					reportError(tok, Severity::error, "type assign", "Violate rule R-1-10-5: Forbid variable assigned to Pointer.", 0U, false);
				else if (var1->isPointer() && Token::Match(tok->astOperand2(), "& %var%") && !tok->astOperand2()->next()->variable()->isArrayOrPointer())
						reportError(tok, Severity::error, "type assign", "Violate rule R-1-10-4: Pointer assignment must keep type consistent.", 0U, false);
			}
		}
	}
	...
	for (std::list<Scope>::const_iterator scope = symbolDatabase->scopeList.begin(); scope != symbolDatabase->scopeList.end(); ++scope) {
		...
		Token *tok = const_cast<Token*>(scope->classDef);//for
		tok = tok->next();
		if (Token::Match(tok, "( ; ; )"))
			reportError(tok, Severity::error, "infinite loop", "Violate rule R-1-9-4: forbid using'for(;;)', use while(1) instead of 'for(;;)'", 0U, false);
		else
		{
			std::vector<Token *> varlist;
			for (Token *vtok = tok; vtok != tok->link(); vtok = vtok->next()){//check for control condition 
				if (vtok->tokType() == Token::eVariable){
					const Variable* var = vtok->variable();
					varlist.push_back(vtok);
					if (!var->isLocal())
						reportError(vtok, Severity::error, "for loop variable", "Violate rule R-1-9-1: for loop variable must be local.", 0U, false);
					if (!var->isIntegralType())
						reportError(vtok, Severity::error, "for loop variable", "Violate rule R-1-9-2: for loop variable must be integer.", 0U, false);
				}
			}
			for (const Token* tok = scope->classStart; tok != scope->classEnd; tok = tok->next()){//check for body
				if (tok->tokType() == Token::eVariable && !tok->variable()->isPointer()){
					for (std::vector<Token *>::size_type i = 0; i < varlist.size(); ++i){
						if (varlist[i]->varId() == tok->varId())
							if (Token::Match(tok->next(), "+=|-=|=|++|--|*=|/=") || Token::Match(tok->previous(), "++|--"))
								reportError(tok, Severity::error, "for loop variable", "Violate rule R-1-9-3: forbid changing the loop control variable in loop body.", 0U, false);
					}
				}
			}
		}
...
static const expr creatExpr(const Token *tok, context & ctx){
	while (tok){
		if (Token::Match(tok, "!")){
			return !ctx.bool_val(creatExpr(tok->astOperand1(), ctx));
		}
		else if (Token::Match(tok, "^")){
			return creatExpr(tok->astOperand1(), ctx) ^ creatExpr(tok->astOperand2(), ctx);
		}
		else if (Token::Match(tok, "~")){
			return ~creatExpr(tok->astOperand1(), ctx);
		}
			...
			if (tok->astOperand1()->str() == "null" || tok->astOperand2()->str() == "null" )
				return ctx.bool_const(tok->astOperand1()->str().data()) != ctx.bool_const(tok->astOperand2()->str().data());
			return creatExpr(tok->astOperand1(), ctx) != creatExpr(tok->astOperand2(), ctx);
		}
		else if (Token::Match(tok, "==") ){
			if (tok->astOperand1()->str() == "null" || tok->astOperand2()->str() == "null")
			...
			if (Token::Match(tok->previous(), "( %num% ) ") && tok->astParent()->str()=="("){
				if (tok->str() == "0")
					return ctx.bool_val(false);
				else if (tok->str() == "1")
					return ctx.bool_val(true);
			}
			else if (tok->str().find(".", 1) != std::string::npos)
				return ctx.real_val(tok->str().data());
			return ctx.int_val(tok->str().data());
		}
		else if (Token::Match(tok, "%var%") && !tok->function()){
			const Variable* var = tok->variable();
			switch (var->_dataType){
				case Variable::dataType::IsIntType:
				case Variable::dataType::IsLongType:
				case Variable::dataType::IscharType:
				case Variable::dataType::IsshortType:
					return ctx.int_const(tok->str().data());
					break;
				case Variable::dataType::IsFloatType:
				case Variable::dataType::IsDoubleType:
					return ctx.real_const(tok->str().data());
					break;
				case Variable::dataType::IsboolType:
					return ctx.bool_const(tok->str().data());
					break;
			}
			return ctx.bool_const("const");
		}
		return ctx.bool_const("const");
	}
}
...
static void checkCondition(const Token *starttok, const Token *endtok, expr condi, context & ctx){
	std::vector<Token*> unsignedTok;
	std::vector<expr> ifElseCondition;
	bool isInIfScope = false;
	for (const Token *tok = starttok; tok != endtok; tok = tok->next()) {
		if (Token::Match(tok, "while (") && !Token::Match(tok, "while ( %num% )")&& tok->next()->astOperand2()){
			expr cond = creatExpr(tok->next()->astOperand2(), ctx);
			solver s(ctx);
			assert(condi.is_bool());
			s.add(condi);
			s.push();
			for (Token* stok = const_cast<Token* >(tok->tokAt(2)); stok != tok->tokAt(2)->link(); stok = stok->next()){
				if (stok->tokType() == Token::eVariable){
					const Variable *var = stok->variable();
					if (var && (var->typeEndToken()->isUnsigned() || var->typeStartToken()->isUnsigned())){
						assert((ctx.int_const(stok->str().data()) >= 0).is_bool());
						s.add(ctx.int_const(stok->str().data()) >= 0);
						s.push();
					}
				}
			}
			...
			s.add(cond);
			//std::cout << "while:" << s << "\n";
			//std::cout << "while:" << s.check() << "\n";
			switch (s.check()) {
			case unsat:
				std::cout << "<error> " << "Violate rule R-1-8-2: Unreachable branch.\n";
				//reportError(tok, Severity::error, "Unreachable branch", "Violate rule R-1-8-2: Unreachable branch.", 0U, false);
				break;
			}
			checkCondition(tok->next()->link()->next(), tok->next()->link()->next()->link(), cond, ctx);
			tok = tok->next()->link()->next()->link();
			continue;
		}
		else if (Token::Match(tok, "if (") && tok->next()->astOperand2()){
			expr cond = creatExpr(tok->next()->astOperand2(), ctx);
			ifElseCondition.push_back(cond);
			solver s(ctx);
			assert(condi.is_bool());
			s.add(condi);
			s.push();
			isInIfScope = true;
			for (Token* stok = const_cast<Token* >(tok->next()); stok != tok->next()->link(); stok = stok->next()){
				if (stok->tokType() == Token::eVariable){
					const Variable *var = stok->variable();
					if(var && (var->typeEndToken()->isUnsigned() || var->typeStartToken()->isUnsigned())){
						assert((ctx.int_const(stok->str().data()) >= 0).is_bool());
		...
		else if (Token::Match(tok, "else if (") && tok->tokAt(2)->astOperand2()){
			expr cond = creatExpr(tok->tokAt(2)->astOperand2(), ctx);
			ifElseCondition.push_back(cond);
			solver s(ctx);
			assert(condi.is_bool());
			s.add(condi);
			...
			checkCondition(tok->tokAt(2)->link()->next(), tok->tokAt(2)->link()->next()->link(), cond, ctx);
			tok = tok->next()->link()->next()->link();
			if (!Token::Match(tok, "else if (") || !Token::Match(tok, "else {"))
				isInIfScope = false;
			continue;
			...
			expr cond = condi;
			solver s(ctx);
			for (std::vector<expr>::size_type iter = 0; iter < ifElseCondition.size(); ++iter) {
				assert((ifElseCondition[iter]).is_bool());
				s.add(!ifElseCondition[iter]);
				s.push();
			}
			for (std::vector<Token*>::size_type iter = 0; iter < unsignedTok.size(); ++iter) {
				assert((ctx.int_const(unsignedTok[iter]->str().data()) >= 0).is_bool());
				s.add( ctx.int_const(unsignedTok[iter]->str().data()) >= 0);
				s.push();
			}
			assert(cond.is_bool());
			s.add( cond);
			//std::cout << "else:" << s << "\n";
			//std::cout << "else:" << s.check() << "\n";
			switch (s.check()) {
			case unsat:
				std::cout << "<error> " << "Violate rule R-1-8-2: Unreachable branch.\n";
				//reportError(tok, Severity::error, "unreachable branch", "Violate rule R-1-8-2: Unreachable branch.", 0U, false);
			...
			checkCondition(tok->next(), tok->next()->link(), cond, ctx);
			tok = tok->next()->link();
			isInIfScope = false;
			continue;
		}
		if (!isInIfScope)
			ifElseCondition.clear();
	}
}

void CheckOther::checkBranch(){
	...
	for (std::list<Scope>::const_iterator scope = symbolDatabase->scopeList.begin(); scope != symbolDatabase->scopeList.end(); ++scope) {
		...
		assert(cond.is_bool());
		try {
			checkCondition(scope->classStart, scope->classEnd, cond, ctx);
		}
		catch (exception & ex) {
